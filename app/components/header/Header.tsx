...
export default function Header() {
  ...
  return (
    <>
      <div className={`col-span-8 layout-grid sticky top-0 z-50 py-8 mix-blend-difference`}>
        ...
        <div className={`col-start-7 w-fit h-fit ml-auto ${menuActive ? "hidden" : ""}`}>
            <BracketButton disabled={false} width={"4.3rem"} onClick={() => {setOpen(!open)}} text='MENU'/>
        </div>
      ...
      <div className={`fixed left-0 top-0 w-full h-screen z-40 layout-grid transition-all duration-500 ${open ? "backdrop-blur-md delay-250" : "backdrop-blur-0 pointer-events-none delay-500"}`}></div>
    </>
  )
}
