...
export default function BracketButton({ width, className, disabled, onClick, text, disabledStyle = true, brackets = true }: BracketButtonProps) {
  
  return (
    <div className={`relative ${brackets ? "mx-3" : ""} ${disabled ? "" : "cursor-pointer group"} ${className}`} onClick={() => {if (!disabled) onClick()}}>
        {brackets && <span className={`absolute -left-3 top-0 text-white text-2xl ${disabled && disabledStyle ? 'opacity-25' : ''}`}>[</span>}
        ...
        {brackets && <span className={`absolute -right-3 top-0 text-white text-2xl ${disabled && disabledStyle ? 'opacity-25' : ''}`}>]</span>}
    </div>
  )
}
