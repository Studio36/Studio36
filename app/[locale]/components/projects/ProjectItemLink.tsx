...
export default function ProjectItemLink({ image, title, id, setIsActive, setIsLinkClicked }: ProjectItemLinkProps) {
  return (
    <Link setIsLinkClicked={setIsLinkClicked} href="/photoset/[id]" id={id} className='rounded-[0.25rem] lg:rounded-lg aspect-[0.67] w-full overflow-hidden block' onMouseEnter={() => {setIsActive(true)}} onMouseLeave={() => {setIsActive(false)}}>
