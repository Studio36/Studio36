...
export default function RulesAndBenefits() {
  ...
  const rulesArray = [ ...Array(11).keys() ];
                ...
                <motion.ol className="col-start-1 col-span-3 lg:col-span-2 list-decimal ml-5 lg:ml-11" whileInView={'animate'} initial={'initial'} variants={textContainerVariants}>
                    {
                      rulesArray.map((value, index) => {
                        return (
                          <Fragment key={index}>
                            <motion.li variants={textVariants} className="font-hedwig text-sm lg:text-2xl font-normal text-justify">{t(`rules.${index}`)}</motion.li> <br/>
                          </Fragment>
                        )
                      })
                    }
